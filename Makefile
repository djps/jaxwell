...
release:          ## Create a new tag for release.
	...
	@echo "$${TAG}" > jaxwell/VERSION
	@gitchangelog > HISTORY.md
	@git add jaxwell/VERSION HISTORY.md
...
test:             ## Run tests and generate coverage report.
	$(ENV_PREFIX)coverage run --source=jaxwell -m pytest -vs
...
testenv:          ## Create a test environment.
	...
	@python -c "import sys; assert sys.version_info >= (3, 7), 'Python 3.7 or higher is required'" || exit 1
	@rm -rf .venv
	@python3 -m venv .venv
